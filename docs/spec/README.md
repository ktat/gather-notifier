...
### v2.1 - 通知音タイプ選択と設定保持（実装完了）
- **通知音タイプ選択**: 4種類の通知音パターンから選択可能（ショート、ロング、ショート（速い）、ロング（速い））
- **ロングトーン実装**: `playLongSound()`関数を追加。5回連続のビープ音（800Hz→600Hz下降トーン）を生成
- **再生間隔の制御**: soundTypeに応じてループ間隔を動的に切り替え（通常/速い）
- **設定保持**: `onInstalled`イベントでマージ方式を採用し、更新時に既存設定を保持
- **新規ストレージキー**: `soundType`（string、デフォルト: 'short'）を追加
- **多言語対応**: soundType関連の5つのi18nキーを4言語すべてに追加
