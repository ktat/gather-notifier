...
9. **集中モード**: 一定時間通知を無効化する応答不可モード機能
10. **設定管理**: 通知種別の個別有効/無効切り替え（Wave、Chat、Call、カレンダー）
11. **バージョン選択**: タブ作成時にGather V1またはV2を選択可能
12. **自動タブ作成**: Gather.townタブが見つからない場合の自動タブ作成（選択したバージョンを使用）
13. **言語選択**: ポップアップでの手動言語選択ドロップダウン（自動/英語/日本語/ドイツ語/フランス語）
14. **デバッグモード**: トラブルシューティング用のデバッグログ切り替え（ポップアップ左下のチェックボックスで有効化）
15. **国際化対応**: ブラウザの言語設定または手動選択に基づいて自動的に適切な言語でメッセージを表示
